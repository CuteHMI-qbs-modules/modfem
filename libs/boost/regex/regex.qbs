...
Module {
	...
	Properties {
		...
		cpp.dynamicLibraries: ["boost_regex"]
	}

	Probes.PathProbe {
		id: libraryProbe

		names: ["libboost_regex"]
		nameSuffixes: [".so"]
		//<qbs-cutehmi.libs.boost-1.workaround target="Linux_distributions" cause="stale">
		...
		//					.concat(cpp.distributionLibraryPaths ? cpp.distributionLibraryPaths : [])
		//					.concat([cutehmi.dirs.externalLibDir])
		//</qbs-cutehmi.libs.boost-1.workaround>
	}

	Probes.PathProbe {
		id: headerProbe

		names: ["boost/regex.hpp"]
		//<qbs-cutehmi.libs.boost-1.workaround target="Linux_distributions" cause="stale">
		// Unless packages are upgraded deprecated features such as "pathPrefixes" have to be used.
